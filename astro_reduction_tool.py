...
def read_path_into_CCDData(data_p, keys, filt=""):
    ...
    if os.path.isdir(data_p):
        ...
        if len(ccd_p.files_filtered(filter=filt)) != 0:
            logger.info(
                "Loading data from: %s len = %d",
                data_p,
                len(ccd_p.files_filtered(filter=filt)),
            )
            if filt != "":
            ...
            for h in range(0, len(ccd_p)):
                ...
                ccd_file = CCDData(
                    fits.getdata(dir),
                    meta=fits.getheader(dir, keywords=keys),
                    unit="adu",
                )
...
def folders_reduction(dir_folder, m_dark, m_bias, m_flat_list, filt, d):
    for f in filt:
        ...
        if len(ccd_folder) != 0:
            ...
            # ccdp.subtract_bias(ccd_file, m_bias)
            ...
            # #     scale=True,
            # # )
            # ccdp.flat_correct(ccd_file, m_flat_list[f])
            now = datetime.now()
            formatted_time = now.strftime("[%Y-%m-%dT%H:%M:%S.%f]")
...
if __name__ == "__main__":
    ...
    for d in folders:
        folders_reduction(
            args.dir, args.master_dark, args.master_bias, m_flat_list, flat_filters, d
        )
